...
| 5 `cuadernillo_id` del activo de la sesión | **Aplicado el 2026-08-23** tras verlo en producción: el primer alumno real entró antes de la primera publicación y sus 14 intentos llegaron con `cuadernillo_id` vacío. Ahora `custom.js` manda `cuadernillo` (el nombre del archivo abierto, sin `.ipynb` ni `_vN`) y el puente lo usa, con el activo del arranque como respaldo y validación del nombre. |
