...
class Exchange(ABCExchange):
    ...
    def _assignment_not_found(self, src_path, other_path):
        ...
        if found:
            ...
            # AVA: fuzzywuzzy no viene en la imagen; sin este guardia el
            # ImportError taparia el error real ("Assignment not found").
            try:
                from fuzzywuzzy import fuzz
            except ImportError:
                self.log.error("Existen: %s", ", ".join(sorted(found)))
            else:
